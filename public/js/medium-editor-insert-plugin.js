...
this["MediumInsert"]["Templates"]["src/js/templates/embeds-wrapper.hbs"] = Handlebars.template({"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
    ...
    // Comment out medium-insert-embeds-overlay to allow clicking on iframes
...
; (function ($, window, document, undefined) {
    ...
    var pluginName = 'mediumInsert',
        ...
        defaults = {
            ...
            placeholder: 'Paste a YouTube or SoundCloud link and press Enter',
            oembedProxy: '//iframe.ly/api/oembed?iframe=1&api_key=10a55901763c6ff4860fed',
            captions: true,
    ...
    Embeds.prototype.parseUrl = function (url, pasted) {
        ...
        if (!(new RegExp(['youtube', 'youtu.be', 'vimeo', 'instagram', 'twitter', 'facebook', 'soundcloud'].join('|')).test(url))) {
...
; (function ($, window, document, Util, undefined) {
    ...
    Images.prototype.uploadAdd = function (e, data) {
        ...
        xhr.upload.onprogress = function(event) {
            $.proxy(that, 'uploadProgress', event, data)();
        }
        xhr.onreadystatechange = function()
        {
            if (xhr.readyState == 4 && xhr.status == 200)
            {
                $.proxy(that, 'uploadDone', url, data)();
            }
        }; 
    ...
    Images.prototype.uploadProgress = function (e, data) {
        ...
        if (this.options.preview) {
            progress = 100 - parseInt(e.loaded / e.total * 100, 10);
