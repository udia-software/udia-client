...
export default class CryptoManager {
  ...
  /**
   * Helper utility function to generate encrypted items for the API.
   * @param decryptedPayload Decrypted Note or Directory belonging to the user
   * @param encPrivateSignKey string encrypted private signing key of the user
   * @param encSecretKey string encrypted secret key of the user
   * @param akB64 ak secret generated by derivation, as base 64 encoded string
   * @param mkB64 mk secret generated by derivation, as base 64 encoded string
   * @param setLoadingText (optional) update the loading text as the encryption processes
   */
  public async encryptToItem(
    decryptedPayload: DecryptedNote | DecryptedDirectory,
    ...
    setLoadingText("Decrypting signing key...");
    const akBuf = Buffer.from(akB64, "base64");
    const rawSignKey = await this.decryptWithSecret(
      encPrivateSignKey,
      akBuf.buffer
    );
    const signKey = await this.importPrivateSignJsonWebKey(
    ...
    setLoadingText("Signing the raw note...");
    const noteData = JSON.stringify(decryptedPayload);
    const noteArrBuf = Buffer.from(noteData, "utf8").buffer;
    const sigArrBuf = await this.signWithPrivateKey(signKey, noteArrBuf);
    // Generate new AES-GCM key for this secret note
    setLoadingText("Generating item AES-GCM key...");
    const itemKey = await this.generateSymmetricEncryptionKey();
    // Encrypt the note with newly generated symmetric encryption key
    setLoadingText("Encrypting the signed note...");
    const encNoteContent = await this.encryptWithSecretKey(
      noteArrBuf,
      itemKey,
      sigArrBuf
    );
    // Encrypt the item key user's secret key
    setLoadingText("Encrypting the item encryption key...");
    const itemJWK = await this.exportJsonWebKey(itemKey);
    const stringifiedItemKey = JSON.stringify(itemJWK);
    const itemKeyArrBuf = Buffer.from(stringifiedItemKey, "utf8").buffer;
    const encItemKey = await this.encryptWithSecretKey(
      itemKeyArrBuf,
      secretKey
    );
    return { encNoteContent, encItemKey };
  }

  /**
   * Helper utility function for decrypting items returned from the API.
   * @param item API returned raw Item object belonging to the user
   * @param encSecretKey string encrypted secret key of the user
   * @param pubVerifyKey string public verification key of the user
   * @param akB64 ak secret generated by derivation, as base 64 encoded string
   * @param mkB64 mk secret generated by derivation, as base 64 encoded string
   * @param setLoadingText (optional) update the loading text as the encryption processes
   */
  public async decryptFromItem(
    item: Item,
    encSecretKey: string,
    strPubVerifyKey: string,
    akB64: string,
    mkB64: string,
    setLoadingText: (loadingText: string) => void = () => undefined
  ) {
    ...
    if (!item.content) {
      throw new Error(`Item ${item.uuid} missing mandatory field 'content'!`);
    }
    ...
    setLoadingText("Importing secret key...");
    const secretKey = await this.importSecretJsonWebKey(
      JSON.parse(Buffer.from(rawSecretKey).toString())
    );
    setLoadingText("Decrypting item key...");
    const rawItemKey = await this.decryptWithSecretKey(
      item.encItemKey,
      secretKey
    );
    setLoadingText("Importing item key...");
    ...
    setLoadingText("Decrypting item...");
    const rawNoteContent = await this.decryptWithSecretKey(
      item.content,
      itemKey
    );
    setLoadingText("Verifying item signature...");
    const encPayloadParts = item.content.split(":");
    if (encPayloadParts.length !== 5) {
      throw new Error(
        `EncryptedPayload must be in form "ver:taglenBase64:addBase64:ivBase64:encBase64"`
      );
    }
    const add = Buffer.from(encPayloadParts[2], "base64").buffer;
    const pubVerifyKey = await this.importPublicVerifyJsonWebKey(
      JSON.parse(strPubVerifyKey)
    );
    if (!this.verifyWithPublicKey(pubVerifyKey, add, rawNoteContent)) {
      throw new Error("Failed to verify ECDSA signature!");
    }

    const decryptedContent: DecryptedNote | DecryptedDirectory = JSON.parse(
